...
'''
path = 'D:\Data\\big2data\paipai-master\data\\'
lb = LabelEncoder()
...
def gen_data(args):
    ...
    # path = args['path']
    # 1.训练数据、测试数据
    if os.path.exists(path + 'cache\\train.csv') and os.path.exists(path + 'cache\\test.csv'):
        print('yes')
        train = pd.read_csv(path + 'cache\\train.csv')
        test = pd.read_csv(path + 'cache\\test.csv')
    else:
        if os.path.exists(path + 'cache\\train_cache.csv') and os.path.exists(path + 'cache\\test_cache.csv'):
            print('haha')
            train_cache = pd.read_csv(path + 'cache\\train_cache.csv')
            test_cache = pd.read_csv(path + 'cache\\test_cache.csv')
        else:
            train_cache, test_cache = gen_train_data()

        train, test = add_features(train_cache, test_cache)
        train.to_csv(path + 'cache\\train.csv', index=None)
        test.to_csv(path + 'cache\\test.csv', index=None)
...
def gen_train_data():
    ...
    train = pd.read_csv(path+'train.csv')#(path + "train.csv")
    test = pd.read_csv(path+'test.csv')#(path + "test.csv")
    ...
    if not os.path.exists(path + 'cache\\'):
        os.mkdir(path + 'cache\\')
    train.drop(columns=['repay_date', 'repay_amt']).to_csv(path + 'cache\\train_cache.csv', index=None)
    test.to_csv(path + 'cache\\test_cache.csv', index=None)
...
def add_features(train, test):
    ...
    train, test = add_listing_info_features(train, test)
    train, test = add_user_info_features(train, test)
    # train, test = add_user_taglist_features(train, test, path)
    train, test = add_user_behavior_features(train, test)
    train, test = add_user_repay_features(train, test)
...
def add_listing_info_features(train, test):
...
def add_user_info_features(train, test):
...
def add_user_taglist_features(train, test):
...
def add_user_behavior_features(train, test):
...
def add_user_repay_features(train, test):
    print("==========add_user_repay_features==========")
    # 6.用户还款日志表（user_repay_logs.csv）
    user_repay_logs = pd.read_csv(path + "user_repay_logs_1.csv")
